...
5. 出版前最終誠信驗證
6. 輸出格式：MD + DOCX（Pandoc 可用時）+ LaTeX（APA 7.0 `apa7` class / IEEE / Chicago）→ tectonic 編譯 PDF
...
12. **跨模型驗證（可選）** — 用 GPT-5.4 Pro 或 Gemini 3.1 Pro 做誠信抽樣查核與獨立 DA 挑戰；同儕審查第六位 reviewer 仍屬規劃中，尚未實作
...
- 直接產出 `.docx` 需要 Pandoc；LaTeX/PDF 輸出需要 Claude Code + `tectonic`；claude.ai 仍可產出 Markdown 與 DOCX 轉換說明
...
**模式：** full、quick、review、lit-review、fact-check、socratic、**systematic-review**（新增）
...
| Formatter | LaTeX + DOCX（經由 Pandoc）+ PDF 輸出 — 強制 `apa7` class、XeCJK 雙語、`ragged2e` 對齊修正、tectonic 編譯 |
...
**模式：** full、plan、outline-only、revision、revision-coach、abstract-only、lit-review、format-convert、citation-check、**disclosure**
...
**模式：** full、re-review（驗收）、quick、methodology-focus、guided、**calibration**
...
| 5. 定稿 | academic-paper | 詢問格式風格 → MD → Pandoc 可用時產出 DOCX（否則提供說明）→ LaTeX → tectonic → PDF |
